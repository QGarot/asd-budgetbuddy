...
void main() {
  group('DataCubit', () {
    ...
    test(
      'updateBudget updates budget in Firestore and local state',
      () async {
        // Mock FirebaseAuth
        when(mockAuth.currentUser).thenReturn(mockUser);
        when(mockUser.uid).thenReturn('uid123');
        ...
        verify(mockBudgetDocRef.update({
          'name': 'Updated Name',
          'category': 'Updated Category',
        })).called(1);
    ...
    test(
      ...
      () async {
        ...
        final mockBudgetExpensesCol = MockCollectionReference<Map<String, dynamic>>();
        when(mockBudgetDocRef.collection('expenses')).thenReturn(mockBudgetExpensesCol);
        when(mockBudgetExpensesCol.get()).thenAnswer((_) async => mockExpensesQuery);
        ...
        verify(mockWriteBatch.delete(any)).called(2); // Once for the expense, once for the budget
